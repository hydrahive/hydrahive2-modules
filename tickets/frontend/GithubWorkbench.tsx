...
import type { GithubConnection, GithubIssue, GithubLinkedTicket } from "./types"
...
export function GithubWorkbench({ onSelectTicket }: GithubWorkbenchProps) {
  ...
  async function reloadConnection(id = connectionId) {
    ...
    setLoading(true)
    try {
      const [remote, local] = await Promise.all([ticketsApi.githubIssues(id, issueState), ticketsApi.githubLinkedTickets(id)])
      setIssues(remote)
  ...
  useEffect(() => { void reloadConnection() }, [connectionId, issueState])
