...
async def test_reconciliation_marks_found_handoff_consumed(monkeypatch):
    ...
    async def found(connection, marker):
        assert marker == job.handoff_id
        return "SABnzbd_nzo_found"

    monkeypatch.setattr(reconciliation, "find_handoff", found)
...
async def test_reconciliation_requires_manual_review_after_24h(monkeypatch):
    ...
    monkeypatch.setattr(reconciliation, "find_handoff", missing)
    result = await reconciliation.reconcile_job(
