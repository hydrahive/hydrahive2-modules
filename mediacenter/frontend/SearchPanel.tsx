  ...
  return <section className="rounded-[6px] border border-[#28354a] bg-[#101724] p-4">
    <div className="mb-4 flex gap-1 overflow-x-auto" role="tablist" aria-label={t("search.mediaTypes")}>
      {MEDIA_TYPES.map((item) => <button key={item.id} type="button" role="tab"
        aria-selected={mediaType === item.id} disabled={locked} onClick={() => setMediaType(item.id)}
        className={`shrink-0 rounded-[4px] px-3 py-2 text-xs font-bold transition disabled:cursor-not-allowed disabled:opacity-50 ${mediaType === item.id ? "bg-cyan-400/15 text-cyan-200 ring-1 ring-cyan-400/40" : "text-[#8d9ab0] hover:bg-white/[5%] hover:text-[#d4deeb]"}`}>
        {t(`media.${item.id}`)}
      </button>)}
    </div>
    <form onSubmit={submit} className="space-y-3">
      ...
      {advanced && <fieldset disabled={locked} className="grid gap-3 border-t border-[#263247] pt-3 sm:grid-cols-2 lg:grid-cols-4">
