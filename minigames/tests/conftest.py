"""Self-contained Test-Fixtures fürs Minigames-Modul.

Eigenständig (kein Core-conftest); hängt den Score-Router exakt wie der Core
unter /api/modules/minigames an die App.
"""
...
MOD_PREFIX = "/api/modules/minigames"
...
def setup_test_env():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        (tmp_path / "config" / "users.json").write_text(json.dumps({
            ...
            "third": {"password_hash": ph, "role": "user"},
        }, indent=2))

        from hydrahive.api import main
        from backend.score_routes import router as score_router
        main.app.include_router(score_router, prefix=MOD_PREFIX)
        yield tmp_path
...
def auth_headers(client):
    return _headers(client, "testuser")


@pytest.fixture
def other_headers(client):
    return _headers(client, "other")


@pytest.fixture
def third_headers(client):
    return _headers(client, "third")


@pytest.fixture(autouse=True)
def _scores_db(setup_test_env):
    """Migrierte + leere Scores-Tabelle vor jedem Test."""
    from hydrahive.db import init_db
    from hydrahive.db.connection import db
    from hydrahive.modules.migrations import apply_module_migrations

    init_db()
    apply_module_migrations("minigames", MODULE_DIR / "migrations")
    with db() as c:
        c.execute("DELETE FROM module_minigames_scores")
    yield
