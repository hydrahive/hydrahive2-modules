...
import { Download, Music, Pause, Play, Repeat, Repeat1, Shuffle, SkipBack, SkipForward, Trash2, Volume2 } from "lucide-react"
...
export function MusicPlayerProjectView({ projectId }: { projectId: string }) {
  const { t } = useTranslation("musicplayer")
  const [tracks, setTracks] = useState<Track[]>([])
  const [permissions, setPermissions] = useState<LibraryPermissions>({ can_upload: false, can_delete: false })
  const player = useAudioPlayer(projectId, tracks)
  ...
  const load = useCallback(() => {
    musicApi.list(projectId)
      ...
      .catch(() => {
        setTracks([])
        setPermissions({ can_upload: false, can_delete: false })
      })
  }, [projectId])
  useEffect(() => { load() }, [load])
      ...
      <div className="space-y-3">
              ...
              {tracks.map((track, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => void musicApi.download(projectId, track).catch(() => {})}
                      aria-label={t("mp_download", { title: track.title })}
                      title={t("mp_download", { title: track.title })}
                      className="grid h-7 w-7 shrink-0 place-items-center rounded-[4px] text-[#64748b] transition-colors hover:bg-cyan-500/10 hover:text-cyan-200 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#69d7ff]"
                    >
                      <Download size={12} />
                    </button>
        ...
        {permissions.can_upload && (
          <div className="space-y-2 border-t border-[#2a364b] pt-3">
            <UploadButton projectId={projectId} onDone={load} />
            <GeneratedImport projectId={projectId} onImported={load} />
          </div>
        )}
