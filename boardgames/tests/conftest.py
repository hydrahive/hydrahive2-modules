"""Self-contained Test-Fixtures fürs Brettspiele-Modul."""
...
MOD_PREFIX = "/api/modules/boardgames"
...
def setup_test_env():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        from backend.chess_routes import router as chess_router
        from backend.result_routes import router as result_router
        main.app.include_router(result_router, prefix=MOD_PREFIX)
        main.app.include_router(chess_router, prefix=MOD_PREFIX)
        yield tmp_path
...
def _results_db(setup_test_env):
    from hydrahive.db import init_db
    from hydrahive.db.connection import db
    from hydrahive.modules.migrations import apply_module_migrations

    init_db()
    apply_module_migrations("boardgames", MODULE_DIR / "migrations")
    with db() as c:
        c.execute("DELETE FROM module_boardgames_results")
    yield
